...
Using PyMuPDF (fitz) with text repair for disjointed English acronyms.

Usage:
    python extract_text_pymupdf.py                  # Extract all chapters
    python extract_text_pymupdf.py 5                 # Extract only chapter 5
    python extract_text_pymupdf.py 1 3 5             # Extract chapters 1, 3, and 5
...
Output:
    - Files are saved to ./text_pymupdf/第X章_章节名.txt
"""
...
def fix_broken_english(text: str) -> str:
    ...
    """
    # Lookbehind for non-English char (or start), lookahead for non-English (or end)
...
def extract_chapter(
    ...
    """
    Extract a single chapter's text using PyMuPDF.
    ...
    for i in range(start_idx, end_idx + 1):
        if i < len(doc):
            ...
            # Use "blocks" to get text, then join?
            # Or just page.get_text() which is simpler but required the fix function.
            # Using simple text extraction + repair as per plan.
    ...
    for chapter_num, chapter_name, toc_start, toc_end in chapters_to_extract:
        ...
        try:
            output_path = extract_chapter(
                doc, chapter_num, chapter_name, toc_start, toc_end, OUTPUT_DIR
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print("Usage: python extract_text_pymupdf.py [chapter_numbers...]")
            print("Examples:")
            print("  python extract_text_pymupdf.py          # Extract all chapters")
            print("  python extract_text_pymupdf.py 5        # Extract chapter 5")
            print(
                "  python extract_text_pymupdf.py 1 3 5    # Extract chapters 1, 3, 5"
            )
    else:
        extract_chapters()
